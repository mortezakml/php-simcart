...
$file_open='C:\\Users\\morte\\Downloads\\';
...
foreach($csv_file_array as $kay=>$value){
  if (($handle = fopen(($file_open.$value), "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle, 2000, ",")) !== FALSE) {
        ...
        $result = pg_query($conn, "INSERT INTO gspn (imei,sku,serial,filename) VALUES ('{$data[0]}', '{$data[1]}', '{$data[2]}','$value');");
    }
    fclose($handle);
